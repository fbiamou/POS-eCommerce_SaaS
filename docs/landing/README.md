...
- **La 2K n'arrive qu'après.** Sur un écran d'au moins 1800 pixels physiques de large (hors économie de données et 2G/3G), la version 2K (`hero-scrub-2k.mp4`) est téléchargée en arrière-plan une fois la 1080p entièrement en mémoire, puis remplace la vidéo sans coupure (clone caché, calé sur la même image, puis échange). Ne pas charger la 2K en premier : à 4 Mbit/s le démarrage passait de 4-5 s à 6-9 s. Ne pas se fier à `navigator.connection.downlink` pour choisir : Chrome annonce environ 1,5 Mbit/s au chargement quelle que soit la connexion réelle.
