...
export default async function SettingsPage({
  ...
  return (
    <div className="flex flex-col gap-6 max-w-3xl">
      ...
      {/* Tab Navigation */}
      <div className="flex gap-1 rounded-2xl bg-zinc-50 dark:bg-[#2d2936]/50 p-1">
        {tabs.map((tab) => (
          <a
            key={tab.id}
            href={`?tab=${tab.id}`}
            className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl text-[13px] font-bold transition-all ${
              activeTab === tab.id
                ? 'bg-white dark:bg-[#1C1A22] text-zinc-900 dark:text-white shadow-sm ring-1 ring-zinc-200/50 dark:ring-white/10'
      ...
      {activeTab === 'profil' && (
        <div className="flex flex-col gap-6">
          ...
          <section className="rounded-2xl border border-zinc-100 bg-white dark:border-[#2d2936] dark:bg-[#1C1A22] shadow-sm overflow-hidden">
            <div className="px-6 py-4 border-b border-zinc-100 dark:border-[#2d2936] bg-transparent">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="md:col-span-2 flex flex-col gap-1.5">
                  ...
                  <textarea id="shop_address" name="shop_address" rows={2} defaultValue={shopSettings?.shop_address || ''} placeholder={t('address_placeholder')}
                    className="w-full rounded-xl border border-zinc-200 bg-white px-4 py-2.5 text-[13px] font-medium transition-colors focus:border-violet-500 focus:outline-none focus:ring-1 focus:ring-violet-500 dark:border-[#2d2936] dark:bg-[#1C1A22] resize-none" />
                </div>
              </div>

              <div className="border-t border-zinc-100 dark:border-[#2d2936] pt-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div className="md:col-span-2 flex items-center gap-3">
                    <input
                      ...
                      className="h-4 w-4 rounded border-zinc-300 text-violet-600 focus:ring-violet-500 dark:border-[#2d2936] dark:bg-[#1C1A22]"
                    />
                    ...
                    <input id="vat_rate" name="vat_rate" type="number" step="0.01" min="0" max="100"
                      ...
                      className="w-24 rounded-xl border border-zinc-200 bg-white px-3 py-2 text-[13px] font-medium transition-colors focus:border-violet-500 focus:outline-none focus:ring-1 focus:ring-violet-500 dark:border-[#2d2936] dark:bg-[#1C1A22]" />
                ...
                {ACCENT_COLORS.map((color) => (
                  <label key={color.value} className="cursor-pointer group" title={t(color.key)}>
                    <input type="radio" name="theme_accent_color" value={color.value}
                      ...
                      className="sr-only" />
                    <div className={`h-9 w-9 rounded-full ${color.className} flex items-center justify-center ring-2 ring-offset-2 ring-transparent group-has-[:checked]:ring-current transition-all dark:ring-offset-[#1C1A22]`}>
